...
from app.core.database import create_view_if_not_exists
...
class DictionaryService:
    ...
    def create_character(
        ...
        if cursor.fetchone():
            raise ValueError(f"ID {character.id} 已存在")
        ...
        # 插入读音信息
        if character.on_readings:
            for i, reading in enumerate(character.on_readings):
                cursor.execute('''
                INSERT INTO japanese_readings (
                    character_id, reading_type, reading_index, reading_value
                ) VALUES (?, 'on', ?, ?)
                ''', (character.id, i, reading))
        
        if character.kun_readings:
            for i, reading in enumerate(character.kun_readings):
                cursor.execute('''
                INSERT INTO japanese_readings (
                    character_id, reading_type, reading_index, reading_value
                ) VALUES (?, 'kun', ?, ?)
        ...
        if character.chinese_readings:
            for i, reading in enumerate(character.chinese_readings):
                cursor.execute('''
        ...
        # 返回创建的汉字信息
        return DictionaryService.get_character_by_id(conn, character.id)
    ...
    def get_all_characters(db: Session) -> List[Dict[str, Any]]:
        ...
        try:
            # 使用视图获取所有汉字信息
            result = db.execute("SELECT * FROM character_view").fetchall()
            return [dict(row) for row in result]
        except Exception as e:
            print(f"Error getting all characters: {str(e)}")
            raise
    ...
    def get_levels(db: Session) -> List[str]:
        """获取所有级别列表"""
        try:
            result = db.execute("SELECT DISTINCT level FROM characters WHERE level IS NOT NULL ORDER BY level").fetchall()
            return [str(row[0]) for row in result if row[0] is not None]
        except Exception as e:
            print(f"Error getting levels: {str(e)}")
            raise

    @staticmethod
    def get_hot_searches(db: sqlite3.Connection, limit: int = 10) -> List[Dict[str, Any]]:
        """获取热门搜索词"""
        cursor = db.cursor()
        cursor.execute("""
    ...
    @staticmethod
    def record_search(db: sqlite3.Connection, search_term: str) -> None:
        """记录搜索词"""
        cursor = db.cursor()
        cursor.execute("""
